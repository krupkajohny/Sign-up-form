...
const inputValidation = (symbols = false, input, msgEl, errMsgStartStr) => {
  ...
  const errRequiredStr = `${
    errMsgStartStr.slice(0, 1).toUpperCase() + errMsgStartStr.slice(1)
  } is required!`;
  ...
  if (symbols && !input.value.trim().match(symbols))
    return displayMsg(msgEl, errFormatStr);
};
...
const passConfirmCheck = (inputPass, msgEl, inputConfirm) => {
  ...
  if (!arrMsgIs && confirmIsNotPass)
    return displayMsg(msgEl, passNotMatchErrStr);
};
